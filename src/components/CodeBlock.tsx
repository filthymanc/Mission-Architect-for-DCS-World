    ...
    return (
        <>
            {parts.map((part, i) => {
                ...
                // Comments
                if (part.startsWith('--')) {
                    return <span key={i} className="text-app-tertiary italic">{part}</span>;
                }
                // Strings
  ...
  return (
    <div className="my-4 rounded-lg overflow-hidden border border-app-border bg-app-frame shadow-sm not-prose">
      <div className="flex items-center justify-between px-4 py-2 bg-app-canvas border-b border-app-border">
        <span className="text-xs font-mono text-app-tertiary uppercase tracking-wider font-bold">{language}</span>
