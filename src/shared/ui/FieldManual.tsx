    ...
    <div className="flex gap-1">
      {keys.map((k, i) => (
        <React.Fragment key={k}>
          ...
          {i < keys.length - 1 && (
            <span className="text-app-tertiary self-center">+</span>
          )}
        </React.Fragment>
      ))}
    </div>
...
const FieldManual: React.FC<FieldManualProps> = ({ isOpen, onClose }) => {
  ...
  const tabs: { id: TabId; label: string; icon: React.ReactNode }[] = [
    ...
    },
    {
      ...
      label: "Controls",
      icon: (
        <path d="M3 5a1 1 0 011-1h12a1 1 0 011 1v10a1 1 0 01-1 1H4a1 1 0 01-1-1V5zm7 1a1 1 0 011 1v7a1 1 0 01-1 1h-2a1 1 0 01-1-1V7a1 1 0 011-1h2z" />
      ),
    },
    {
      id: "tactics",
      ...
      >
        ...
        <div className="w-full md:w-64 bg-app-surface border-b md:border-b-0 md:border-r border-app-border flex flex-col">
          <div className="p-6 border-b border-app-border bg-app-surface/50">
            <h2 className="text-xl font-bold text-app-primary tracking-wide">
              FIELD MANUAL
            </h2>
            <p className="text-xs text-app-tertiary font-mono mt-1">
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar">
            ...
            {activeTab === "briefing" && (
              <div className="space-y-8 max-w-2xl animate-fadeIn">
                <div>
                  <h3 className="text-app-brand font-bold text-sm uppercase tracking-wider mb-2">
                    {content.briefing.concept.subtitle}
                  </h3>
                  <h1 className="text-3xl font-bold text-app-primary mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {content.briefing.features.map((feature, idx) => (
                    <div
                      key={idx}
                      className="p-5 bg-app-canvas rounded-xl border border-app-border"
                    >
            ...
            {activeTab === "systems" && (
              <div className="space-y-8 max-w-2xl animate-fadeIn">
                <div>
                  <h3 className="text-blue-500 font-bold text-sm uppercase tracking-wider mb-4">
                  ...
                  <div className="space-y-4">
                    {content.systems.interface.sections.map((section, idx) => (
                      <div
                        key={idx}
                        className="bg-app-canvas border border-app-border rounded-lg overflow-hidden"
                      >
                        <div className="px-4 py-2 bg-app-surface border-b border-app-border font-bold text-app-primary text-xs uppercase">
                          {section.title}
                        </div>
                        <div className="divide-y divide-app-border">
                          {section.items.map((item, i) => (
                            <div key={i} className="p-3 flex gap-4">
                              <span
                                className={`font-mono text-${item.color}-400 text-xs w-24 shrink-0`}
                              >
                                {item.label}
                              </span>
                ...
                <div>
                  <h3 className="text-blue-500 font-bold text-sm uppercase tracking-wider mb-2">
            ...
            {activeTab === "controls" && (
              <div className="max-w-2xl animate-fadeIn">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                  <div>
                    <h3 className="text-xs font-bold text-app-brand uppercase tracking-wider mb-4">
            ...
            {activeTab === "tactics" && (
              <div className="space-y-8 max-w-2xl animate-fadeIn">
                <div>
                  <h3 className="text-purple-500 font-bold text-sm uppercase tracking-wider mb-2">
                  ...
                  <div className="space-y-3">
                    {content.tactics.prompt.cards.map((card, idx) => (
                      <div
                        key={idx}
                        className={`p-4 bg-app-canvas border border-app-border rounded-xl border-l-4 ${card.type === "trap" ? "border-l-red-500" : "border-l-app-brand"}`}
                      >
                        <strong
                          className={`${card.type === "trap" ? "text-red-400" : "text-app-brand"} text-xs uppercase block mb-1`}
                        >
                          {card.title}
                        </strong>
                  ...
                  <div className="space-y-3">
                    {content.tactics.errors.items.map((item, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className={`p-4 bg-app-canvas border border-app-border rounded-xl border-l-4 ${style.border}`}
                        >
                          <strong
                            className={`${style.title} font-bold text-sm mb-1 block`}
                          >
                            {item.label}
                          </strong>
            ...
            {activeTab === "developer" && (
              <div className="space-y-8 max-w-2xl animate-fadeIn">
                <div>
                  <h3 className="text-app-tertiary font-bold text-sm uppercase tracking-wider mb-2">
                ...
                <div>
                  ...
                  <div className="p-6 bg-app-canvas border border-app-border rounded-xl italic text-app-secondary text-sm leading-relaxed">
                    ...
                    <br />
                    <span className="block mt-4 text-xs font-bold not-italic text-app-tertiary">
            ...
            {activeTab === "intel" && (
              <div className="space-y-6 max-w-2xl animate-fadeIn">
                <h3 className="text-orange-500 font-bold text-sm uppercase tracking-wider mb-2">
            ...
            {activeTab === "legal" && (
              <div className="space-y-8 max-w-2xl animate-fadeIn">
                ...
                {/* Credits Section */}
                <div>
                  <h3 className="text-red-500 font-bold text-sm uppercase tracking-wider mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {content.legal.credits.items.map((credit, idx) => (
                      <div
                        key={idx}
                        className="p-4 bg-app-canvas border border-app-border rounded-xl border-l-4 border-l-app-border"
                      >
                        <div className="font-bold text-app-primary text-sm">
                          {credit.title}
                        </div>
                        <div className="text-xs font-bold text-app-tertiary mt-1">
                          {credit.author}
                        </div>
                        <div className="text-xs text-app-tertiary font-mono mt-2 pt-2 border-t border-app-border/50">
                          {credit.license}
                        </div>
                      </div>
                    ))}
                ...
                {/* License Section */}
                <div>
                  <h3 className="text-app-tertiary font-bold text-sm uppercase tracking-wider mb-4">
