...
import { MenuIcon, SendIcon, TrashIcon } from './components/Icons';
...
const MainLayout: React.FC = () => {
  ...
  const { 
    ...
    deleteSession, renameSession, touchSession, mergeSessions 
  } = useSessionManager();
  ...
  // Chat Engine
  const { sendMessage, stopGeneration, isLoading: isGenerating } = useChatEngine({
    apiKey,
  ...
  const { messagesEndRef, scrollContainerRef, handleScroll, scrollToBottom } = useScrollManager(messages, isGenerating);
  ...
  const handleExport = () => {
    try {
        ...
        sessions.forEach(s => {
             const msgs = localStorage.getItem(`${STORAGE_KEYS.SESSION_PREFIX}${s.id}`);
             if (msgs) {
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e: any) => {
        ...
        try {
            ...
            const { validSessions, validMessages, validSettings } = validateImportData(raw);
            ...
            Object.keys(validMessages).forEach(sid => {
                localStorage.setItem(`${STORAGE_KEYS.SESSION_PREFIX}${sid}`, JSON.stringify(validMessages[sid]));
            });
        ...
        <main className="flex-1 flex flex-col min-w-0 relative h-full">
            ...
            <header className="h-14 border-b border-app-border bg-app-frame/90 backdrop-blur flex items-center justify-between px-4 z-20 shrink-0">
                <div className="flex items-center gap-3">
                    <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 text-app-secondary hover:text-app-primary">
                        <MenuIcon className="h-5 w-5" />
                    </button>
                    <div className="flex flex-col">
                        <h2 className="font-bold text-sm truncate max-w-[150px] md:max-w-xs">
                            {sessions.find(s => s.id === activeSessionId)?.name || 'Mission Architect'}
                        </h2>
                        <div className="flex items-center gap-2 text-[10px] font-mono font-bold">
                             <span className={apiStatus === 'error' || apiStatus === 'offline' ? 'text-red-500' : 'text-emerald-500'}>
                                {apiStatus === 'idle' ? 'READY' : apiStatus.toUpperCase()}
                             </span>
                             {isGenerating && <span className="text-app-tertiary">| PROCESSING</span>}
                        </div>
                    </div>
                </div>
            ...
            >
                <div className="max-w-4xl mx-auto min-h-full flex flex-col">
                    {messages.length === 0 || (messages.length === 1 && messages[0].role === 'model' && !isLoadingData) ? (
                         <Dashboard 
                            settings={settings}
                            apiStatus={apiStatus}
                            sessions={sessions}
