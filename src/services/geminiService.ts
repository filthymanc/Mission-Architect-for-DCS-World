...
const frameworkDocsTool: FunctionDeclaration = {
    ...
    description: "Fetches RAW LUA SOURCE CODE from the official GitHub repositories (MOOSE or DML). Use this to analyze function definitions and header comments directly. Semantic Compression is applied to large files.",
    parameters: {
        type: Type.OBJECT,
        properties: {
            framework: {
...
const sseDocsTool: FunctionDeclaration = {
    name: "get_sse_docs",
    description: "Fetches the Safe Standard Scripting Engine (SSE) Hard Deck Definitions. Use this when the user needs to use standard DCS classes like Group, Unit, Timer, or Trigger. Do not rely on training data for these classes.",
    parameters: {
        type: Type.OBJECT,
        properties: {
            category: {
 ...
 */
export async function* sendMessageStream(chatSession: Chat | null, message: any): AsyncGenerator<GenerateContentResponse, void, unknown> {
  ...
  while (turnCount < maxTurns) {
    ...
    try {
        ...
        let detailedError = e.message || errStr;
        try {
        ...
        const errorMessage = `**LIBRARIAN ERROR:** The Librarian crashed while fetching data.\n\n` +
    ...
    if (toolCalls.length > 0) {
        console.log(`[Librarian] executing ${toolCalls.length} tools. Turn: ${turnCount}`);
        ...
        for (const call of toolCalls) {
            ...
            if (call.name === 'get_framework_docs') {
                ...
                const fingerprint = `${framework}:${module_name}:${branch || ''}`.toUpperCase();
                
                if (toolCallHistory.has(fingerprint)) {
                    console.warn(`[Librarian] Duplicate tool call blocked: ${fingerprint}`);
                    result = "SYSTEM ALERT: You have already fetched this module. Do not fetch it again. Use the data previously provided.";
                } else {
            ...
            // HANDLER: SSE Hard Deck
                ...
                if (toolCallHistory.has(fingerprint)) {
                    result = "SYSTEM ALERT: SSE Definitions for this category are already in context.";
                } else {
                    toolCallHistory.add(fingerprint);
                    if (category === 'All') {
                         ...
                         result = "ERROR: Category not found in Hard Deck. Available: Group, Unit, timer, trigger, coalition.";
                    }
                }
            }
        ...
        if (functionResponses.length > 0) {
            currentTurnMessage = functionResponses.map(fr => ({ functionResponse: fr }));
            continue;
        }
    }
