...
const FieldManual: React.FC<FieldManualProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div 
        ref={modalRef}
        className="bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl max-w-5xl w-full h-[85vh] flex flex-col md:flex-row overflow-hidden"
      >
        {/* SIDEBAR NAVIGATION */}
        <div className="w-full md:w-64 bg-slate-900 border-b md:border-b-0 md:border-r border-slate-800 flex flex-col">
            <div className="p-6 border-b border-slate-800 bg-slate-900/50">
                <h2 className="text-xl font-bold text-white tracking-wide">FIELD MANUAL</h2>
                <p className="text-xs text-slate-500 font-mono mt-1">v{APP_VERSION} Reference</p>
            </div>
            ...
            <div className="p-4 border-t border-slate-800 md:block hidden">
                <button 
                    onClick={onClose}
                    className="w-full py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded font-bold text-xs"
                >
                    CLOSE MANUAL
                </button>
            </div>
        ...
        <div className="flex-1 flex flex-col bg-slate-950 min-w-0">
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar">
                ...
                {/* --- TAB: BRIEFING (Emerald) --- */}
                {activeTab === 'briefing' && (
                    <div className="space-y-8 max-w-2xl animate-fadeIn">
                        <div>
                            <h3 className="text-emerald-500 font-bold text-sm uppercase tracking-wider mb-2">{content.briefing.concept.subtitle}</h3>
                            <h1 className="text-3xl font-bold text-white mb-4">{content.briefing.concept.title}</h1>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                             {content.briefing.features.map((feature, idx) => (
                                 <div key={idx} className="p-5 bg-slate-900/50 rounded-xl border border-slate-800">
                                    <h4 className="font-bold text-white mb-2 flex items-center gap-2">
                                        ...
                                        {feature.iconType === 'lock' && <svg className="h-5 w-5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>}
                                        {feature.title}
                                    </h4>
                        ...
                        <div>
                            <h3 className="text-emerald-500 font-bold text-sm uppercase tracking-wider mb-2">{content.briefing.engine.subtitle}</h3>
                            ...
                            <div className="space-y-4">
                                {content.systems.interface.sections.map((section, idx) => (
                                    <div key={idx} className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
                                        <div className="px-4 py-2 bg-slate-800/50 border-b border-slate-800 font-bold text-white text-xs uppercase">{section.title}</div>
                                        <div className="divide-y divide-slate-800">
                                            {section.items.map((item, i) => (
                                                <div key={i} className="p-3 flex gap-4">
                                                    ...
                                                    <p className="text-slate-400 text-sm">{renderText(item.text)}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                ...
                {activeTab === 'controls' && (
                    <div className="max-w-2xl animate-fadeIn">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                            <div>
                                <h3 className="text-xs font-bold text-emerald-500 uppercase tracking-wider mb-4">{content.controls.navigation.title}</h3>
                ...
                {activeTab === 'tactics' && (
                         ...
                         <div>
                            ...
                            <div className="space-y-3">
                                {content.tactics.prompt.cards.map((card, idx) => (
                                    <div key={idx} className={`p-4 bg-slate-900 border border-slate-800 rounded-xl border-l-4 ${card.type === 'trap' ? 'border-l-red-500' : 'border-l-emerald-500'}`}>
                                        <strong className={`${card.type === 'trap' ? 'text-red-400' : 'text-emerald-400'} text-xs uppercase block mb-1`}>{card.title}</strong>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {content.tactics.errors.items.map((item, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className={`p-4 bg-slate-900 border border-slate-800 rounded-xl border-l-4 ${style.border}`}>
                 ...
                 {activeTab === 'developer' && (
                    <div className="space-y-8 max-w-2xl animate-fadeIn">
                        <div>
                            <h3 className="text-slate-400 font-bold text-sm uppercase tracking-wider mb-2">{content.developer.intent.subtitle}</h3>
                        ...
                        <div>
                            <h3 className="text-slate-400 font-bold text-sm uppercase tracking-wider mb-2">{content.developer.inspiration.subtitle}</h3>
                            <div className="p-6 bg-slate-900 border border-slate-800 rounded-xl italic text-slate-400 text-sm leading-relaxed">
                                {renderText(content.developer.inspiration.text)}
                                <br/>
                                <span className="block mt-4 text-xs font-bold not-italic text-slate-500">{content.developer.inspiration.author}</span>
                            </div>
                        </div>

                        <div>
                            <h3 className="text-slate-400 font-bold text-sm uppercase tracking-wider mb-2">{content.developer.stack.subtitle}</h3>
                     ...
                     <div className="space-y-6 max-w-2xl animate-fadeIn">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {content.intel.links.map((link, idx) => (
                                <a key={idx} href={link.url} target="_blank" rel="noopener noreferrer" className="block p-5 bg-slate-900 border border-slate-800 hover:border-orange-500/50 hover:bg-slate-900/80 rounded-xl transition-all group border-l-4 border-l-orange-500">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <p className="text-xs font-bold text-slate-500 mt-1 mb-2">{link.author}</p>
                                        </div>
                                    </div>
                ...
                {activeTab === 'legal' && (
                    <div className="space-y-8 max-w-2xl animate-fadeIn">
                        ...
                        <div className="pb-8 border-b border-slate-800">
                             ...
                             <div className="space-y-3 text-sm text-slate-400">
                                {content.legal.sovereignty.items.map((item, idx) => (
                                    <p key={idx}><strong className="text-white">{item.label}</strong> {item.text}</p>
                                ))}
                             </div>
                        </div>
                        ...
                        <div>
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {content.legal.credits.items.map((credit, idx) => (
                                    <div key={idx} className="p-4 bg-slate-900 border border-slate-800 rounded-xl border-l-4 border-l-slate-600">
                                        <div className="font-bold text-white text-sm">{credit.title}</div>
                                        <div className="text-xs font-bold text-slate-500 mt-1">{credit.author}</div>
                                        <div className="text-xs text-slate-600 font-mono mt-2 pt-2 border-t border-slate-800/50">{credit.license}</div>
                                    </div>
                                ))}
                             </div>
                        </div>
                        ...
                        {/* License Section */}
                        <div>
                             <h3 className="text-slate-400 font-bold text-sm uppercase tracking-wider mb-4">{content.legal.license.subtitle}</h3>
                             <div className="p-4 bg-slate-950 border border-slate-800 rounded-lg overflow-auto max-h-64 custom-scrollbar">
