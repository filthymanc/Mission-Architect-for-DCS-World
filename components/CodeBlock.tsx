...
const CodeBlock: React.FC<CodeBlockProps> = ({ className, children, inline }) => {
  ...
  // 1. Trust the 'inline' prop from ReactMarkdown.
  // 2. Fallback: If no language is specified AND text is short/no-newlines, assume inline.
  //    This prevents single-tick code from accidentally rendering as a heavy block if props are lost.
  ...
  // UX IMPROVEMENT: "Highlighter Style"
  // Completely removed bg, border, and padding to act like a true text highlighter.
  // Font Size: Exactly 1px smaller than surrounding text (calc(1em - 1px)).
  if (isInline) {
    return (
      <code className="text-emerald-400 font-bold font-mono text-[calc(1em-1px)]">
        {codeText || children}
      </code>
    );
  }
  ...
  // We avoid 'react-syntax-highlighter' to prevent Heap Allocation Failures in constrained environments.
    ...
    return (
        <>
            {parts.map((part, i) => {
                ...
                // Keywords
                if (/^(local|function|end|if|then|else|elseif|for|in|do|while|repeat|until|return|break|true|false|nil|and|or|not)$/.test(part)) {
  ...
  return (
    <div className="my-4 rounded-lg overflow-hidden border border-slate-700 bg-slate-950 shadow-sm not-prose">
      <div className="flex items-center justify-between px-4 py-2 bg-slate-900 border-b border-slate-700">
        <span className="text-xs font-mono text-slate-500 uppercase tracking-wider font-bold">{language}</span>
        
        <div className="flex items-center gap-2">
            <button
            onClick={handleDownload}
            className="text-xs text-slate-400 hover:text-blue-400 transition-colors flex items-center gap-1.5 px-2 py-1 rounded hover:bg-slate-800"
            ...
            <button
            onClick={handleCopy}
            className="text-xs text-slate-400 hover:text-emerald-400 transition-colors flex items-center gap-1.5 px-2 py-1 rounded hover:bg-slate-800"
      ...
      {/* UX FIX: whitespace-pre ensures line breaks in code are respected */}
      <div className="p-4 overflow-x-auto text-sm font-mono leading-relaxed bg-slate-950 whitespace-pre">
